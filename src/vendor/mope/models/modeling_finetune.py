...
class Block(nn.Module):
    ...
    def __init__(self,
                 ...
                 attn_head_dim=None,
                 cos_attn=False,
                 use_moe=False,               # 新增，默认False
                 num_routable_experts=17,     # 新增
                 num_shared_experts=4,        # 新增
                 top_k=1):                    # 新增
        super().__init__()
        ...
        # ── 新增参数：use_moe, num_routable_experts, num_shared_experts, top_k ──
        if use_moe:
            from .moe_ffn import SharedExpertMoE
    ...
    def forward(self, x, physics_label=None, time_ids=None,
                physics_label_soft=None, num_time_bins=None):
        if self.gamma_1 is None:
            x = x + self.drop_path(self.attn(self.norm1(x)))
            if self.use_moe:
                moe_out, aux = self.mlp(
                    self.norm2(x), physics_label, time_ids, physics_label_soft,
                    num_time_bins=num_time_bins)
                x = x + self.drop_path(moe_out)
                self._moe_aux = aux                       # 暂存aux，供外部收集loss
            else:
