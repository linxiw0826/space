...
# Only log on rank 0 to avoid duplicate messages in distributed training
# Check environment variable first (set by torchrun/deepspeed)
local_rank = int(os.environ.get("LOCAL_RANK", -1))
if local_rank <= 0:  # Only log on rank 0 or when not in distributed mode
    logger.info("Trainer._get_train_sampler replaced with custom implementation.")