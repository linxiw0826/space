...
def replace_qwen2_vl_attention_class():
    ...
    # Also patch the locally copied Qwen3-VL implementation (qwenvl) used by this repo.
    # This is needed when training imports `Qwen3VLForConditionalGeneration` from `qwenvl.model.modeling_qwen3_vl`.
    try:
        from qwenvl.model import modeling_qwen3_vl as local_qwen3_vl

        if hasattr(local_qwen3_vl, "Qwen3VLTextAttention"):
            local_qwen3_vl.Qwen3VLTextAttention.forward = qwen3vl_forward
        # Local module imports `create_causal_mask` into its namespace at import time. When we use varlen
        # FlashAttention (cu_seqlens passed as 1D attention_mask), we must not call the HF mask builder.
        # Replace it with a passthrough to keep `attention_mask` as-is.
        if hasattr(local_qwen3_vl, "create_causal_mask"):
            local_qwen3_vl.create_causal_mask = return_mask
    except Exception as e:
        logger.warning_once(f"Failed to patch local qwenvl Qwen3-VL attention classes: {e}")
