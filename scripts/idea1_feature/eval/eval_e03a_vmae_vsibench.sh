#!/bin/bash
# =============================================================================
# Evaluate E-03a_vmae (feed-only, VideoMAE frozen encoder) on VSIBench
# Model type: qwen3_vl_mope_crossattn
#
# Usage:
#   bash eval_e03a_vsibench.sh [CKPT_PATH] [EXP_NAME]
#
# Arguments (optional — derived from MODEL_SIZE when omitted):
#   CKPT_PATH  — path to the E-03a checkpoint directory to evaluate
...
case "${MODEL_SIZE}" in
    4b)
        ...
        DEFAULT_EXP_NAME="e03a_feedonly_vmae_4b"
        ;;
    8b)
        DEFAULT_CKPT_PATH="${SPACE_OUTPUT_ROOT}/train/e03a_feedonly_vmae_8b"
        DEFAULT_EXP_NAME="e03a_feedonly_vmae_8b"
        ;;
    *)
...
mkdir -p "${LOG_DIR}"
LOG_FILE="${LOG_DIR}/e03a_vmae_vsibench_${MODEL_SIZE}_$(date +%Y%m%d_%H%M%S).log"
...
# Status output
# ---------------------------------------------------------------------------
echo "=== VSIBench Evaluation (E-03a_vmae feed-only, VideoMAE encoder, MoPE-ON) ==="
