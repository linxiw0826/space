#!/bin/bash
# =============================================================================
# Evaluate E-03a_vmae (feed-only, VideoMAE frozen encoder) on VLM4D
# Model type: qwen3_vl_mope_crossattn
# (forked from eval_e03a_vsibench.sh — only the task + data paths differ).
#
...
#
# Usage:
#   bash eval_e03a_vlm4d.sh [CKPT_PATH] [EXP_NAME]
#
...
# E-03a_vmae: build the VideoMAE encoder at eval (MUST match the checkpoint's
...
case "${MODEL_SIZE}" in
    4b)
        DEFAULT_CKPT_PATH="${SPACE_OUTPUT_ROOT}/train/e03a_feedonly_vmae_4b"
        DEFAULT_EXP_NAME="e03a_feedonly_vmae_4b_vlm4d"
        ;;
    8b)
        DEFAULT_CKPT_PATH="${SPACE_OUTPUT_ROOT}/train/e03a_feedonly_vmae_8b"
        DEFAULT_EXP_NAME="e03a_feedonly_vmae_8b_vlm4d"
        ;;
    *)
...
mkdir -p "${LOG_DIR}"
LOG_FILE="${LOG_DIR}/e03a_vmae_vlm4d_${MODEL_SIZE}_$(date +%Y%m%d_%H%M%S).log"
...
# Status output
# ---------------------------------------------------------------------------
echo "=== VLM4D Evaluation (E-03a_vmae feed-only, VideoMAE encoder, MoPE-ON) ==="
