#!/bin/bash
# =============================================================================
# Evaluate E-01 (GUIDE fine-tuned on VSI-590K, no MoPE) checkpoint on ScanQA
# (forked from eval_e01_vsibench.sh — only the task + data paths differ).
#
# Usage:
#   bash eval_e01_scanqa.sh [CKPT_PATH] [EXP_NAME]
#
# Env vars (all optional, have defaults):
#   MODEL_SIZE           — "4b" or "8b" (default: 4b)
#   SPACE_ROOT           — project root on the execution server
#   SCANQA_VIDEO_ROOT    — directory containing ScanNet video files (read by utils.py)
#   SCANQA_JSONL         — path to ScanQA val jsonl annotation file
#   GUIDE_LMMS_EVAL      — path to GUIDE's lmms-eval repo root
#   RESULTS_DIR          — base directory for evaluation results
#   LOG_DIR              — directory for eval log files
#   CUDA_VISIBLE_DEVICES — GPUs to use (default: 0,1,2,3)
#   NUM_PROCESSES        — number of accelerate processes (default: 4)
# =============================================================================
set -e
source "$(dirname "${BASH_SOURCE[0]}")/../../_common/env/activate.sh"

# --- New-server runtime workarounds ---------------------------------------
if [ -n "${LD_LIBRARY_PATH}" ]; then
    export LD_LIBRARY_PATH="$(printf '%s' "${LD_LIBRARY_PATH}" | tr ':' '\n' | grep -v 'videorepa' | paste -sd: -)"
fi

SPACE_ROOT=${SPACE_ROOT:-"/home/nvme03/wlx/Space_sensing/projects/space"}

MODEL_SIZE=${MODEL_SIZE:-4b}

case "${MODEL_SIZE}" in
    4b)
        DEFAULT_CKPT_PATH="${SPACE_OUTPUT_ROOT}/train/e01_guide_4b"
        DEFAULT_EXP_NAME="e01_guide_4b_scanqa"
        ;;
    8b)
        DEFAULT_CKPT_PATH="${SPACE_OUTPUT_ROOT}/train/e01_guide_8b"
        DEFAULT_EXP_NAME="e01_guide_8b_scanqa"
        ;;
    *)
        echo "ERROR: MODEL_SIZE must be '4b' or '8b', got '${MODEL_SIZE}'"
        exit 1
        ;;
esac

CKPT_PATH=${1:-${DEFAULT_CKPT_PATH}}
EXP_NAME=${2:-${DEFAULT_EXP_NAME}}

GUIDE_LMMS_EVAL=${GUIDE_LMMS_EVAL:-${SPACE_ROOT}/src/vendor/lmms-eval}
GUIDE_TRAIN_ROOT=${SPACE_ROOT}/src

# ScanQA data paths
SCANQA_VIDEO_ROOT=${SCANQA_VIDEO_ROOT:-/data2/wlx/data/VSIBench/scannet}
SCANQA_JSONL=${SCANQA_JSONL:-/data2/wlx/data/ScanQA/scanqa_val.jsonl}
export SCANQA_VIDEO_ROOT  # utils.py 直接从 env 读 media_dir

# Inject the annotation path into the task yaml's data_files.test line so that
# SCANQA_JSONL takes effect (the yaml hardcodes a default like vsibench does).
SCANQA_TASK_YAML="${GUIDE_LMMS_EVAL}/lmms_eval/tasks/scanqa/scanqa.yaml"
if [ -f "${SCANQA_TASK_YAML}" ]; then
    sed -i -E "s#^(    test: ).*#\1${SCANQA_JSONL}#" "${SCANQA_TASK_YAML}"
fi

# Results output
RESULTS_DIR=${RESULTS_DIR:-${SPACE_OUTPUT_ROOT}/eval/scanqa}
OUTPUT_PATH="${RESULTS_DIR}/${EXP_NAME}"
mkdir -p "${OUTPUT_PATH}"

# Log file
LOG_DIR=${LOG_DIR:-${SPACE_LOG_ROOT}/eval}
mkdir -p "${LOG_DIR}"
LOG_PREFIX="${LOG_PREFIX:-e01}"
LOG_FILE="${LOG_DIR}/${LOG_PREFIX}_scanqa_${MODEL_SIZE}_$(date +%Y%m%d_%H%M%S).log"

# GPU configuration
export CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-0,1,2,3}
NUM_PROCESSES=${NUM_PROCESSES:-4}
MAIN_PORT=${MAIN_PORT:-$(shuf -i 20001-29999 -n 1)}

export PYTHONPATH="${GUIDE_LMMS_EVAL}:${GUIDE_TRAIN_ROOT}:${PYTHONPATH}"
export NCCL_NVLS_ENABLE=0

# Model args — standard GUIDE model (qwen3_vl_my), no MoPE
MODEL_ARGS="pretrained=${CKPT_PATH},max_pixels=268324,min_pixels=8192,attn_implementation=flash_attention_2"

echo "=== ScanQA Evaluation (E-01 GUIDE fine-tune, no MoPE) ==="
echo "Model size : ${MODEL_SIZE}"
echo "Checkpoint : ${CKPT_PATH}"
echo "Experiment : ${EXP_NAME}"
echo "Output     : ${OUTPUT_PATH}"
echo "Log        : ${LOG_FILE}"
echo "Video root : ${SCANQA_VIDEO_ROOT}"
echo "JSONL      : ${SCANQA_JSONL}"
echo "Processes  : ${NUM_PROCESSES}  Port: ${MAIN_PORT}"
echo "============================================================"

echo "Starting accelerate launch..."

cd "${GUIDE_LMMS_EVAL}"

accelerate launch \
    --num_processes=${NUM_PROCESSES} \
    --main_process_port ${MAIN_PORT} \
    -m lmms_eval \
    --model qwen3_vl_my \
    --model_args "${MODEL_ARGS}" \
    --tasks scanqa \
    --batch_size 1 \
    --log_samples \
    --log_samples_suffix "${EXP_NAME}" \
    --output_path "${OUTPUT_PATH}" \
    --force_simple \
    2>&1 | tee "${LOG_FILE}"

echo ""
echo "=== Evaluation complete. Results at: ${OUTPUT_PATH} ==="
echo "=== Log saved to: ${LOG_FILE} ==="
