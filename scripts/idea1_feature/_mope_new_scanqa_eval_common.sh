...
if [[ "${MOPE_NEW_EXPERIMENT}" == "e04a-quarter" || "${MOPE_NEW_EXPERIMENT}" == "e05a-quarter" ]]; then
  ...
  DEFAULT_MOPE_NEW_CKPT=/data2/mope-jepa-assets/jepa_checkpoints/native_mope_b_dense8_moe8_top1_shared1_anchor1_final515k_3dpos_ep100_warm3_cos_lr75e6_min25e6/checkpoint-73-posttrain-physbench-fullmope-epoch44.pth
else
...
SCANQA_VIDEO_ROOT="${SCANQA_VIDEO_ROOT:-/data2/wlx/data/VSIBench/scannet}"
SCANQA_JSONL="${SCANQA_JSONL:-/data2/wlx/data/ScanQA/scanqa_val.jsonl}"
QUARTER_MANIFEST="${MOPE_NEW_SPAR_ANN:-/data2/wlx/data/vsi590k_processed/vsi590k_spar_590k_quarter_stratified.json}"
export SCANQA_VIDEO_ROOT
...
if [[ "${NAME}" != "${DEFAULT_NAME}" ]]; then
  [[ "${MOPE_NEW_EXPERIMENT}" == "e04a-new" && "${NAME}" == "e04a_mope_new_e01_projector_only_lr1e4_constant_bs2_diag_4b" ||
     ...
     "${MOPE_NEW_EXPERIMENT}" == "e05a-quarter" && "${NAME}" == "e05_quarter_epoch4" ||
     "${MOPE_NEW_EXPERIMENT}" == "e05a-quarter" && "${NAME}" == "e05_quarter_epoch5" ||
     "${MOPE_NEW_EXPERIMENT}" == "e05a-quarter" && "${NAME}" == "e05_quarter_epoch6" ||
     "${MOPE_NEW_EXPERIMENT}" == "e05a-quarter" && "${NAME}" == "e05_quarter_epoch7" ||
...
FORMAL_RESULTS_DIR="${OUTPUT_ROOT}/eval/scanqa/${NAME}"
...
LOG_FILE="${LOG_FILE:-${LOG_DIR}/${NAME}_scanqa_$(date +%Y%m%d_%H%M%S).log}"
EXPECTED_SAMPLE_COUNT=4675
NUM_PROCESSES="${NUM_PROCESSES:-4}"
MAIN_PORT="${MAIN_PORT:-29528}"
...
if [[ "${ALLOW_MISSING}" != "1" ]]; then
  ...
  [[ -f "${SCANQA_JSONL}" ]] || { echo "Missing ScanQA annotations: ${SCANQA_JSONL}" >&2; exit 2; }
  [[ -d "${SCANQA_VIDEO_ROOT}" ]] || { echo "Missing ScanQA video root: ${SCANQA_VIDEO_ROOT}" >&2; exit 2; }
fi
...
COMMAND=(accelerate launch "--num_processes=${NUM_PROCESSES}" "--main_process_port=${MAIN_PORT}"
  ...
  --include_path "${TASK_DIR}" --tasks scanqa_mope_new
...
echo "ScanQA=${SCANQA_JSONL} video_root=${SCANQA_VIDEO_ROOT} output=${RESULTS_DIR}"
echo "Log=${LOG_FILE}"
...
cp "${GUIDE_LMMS_EVAL}/lmms_eval/tasks/scanqa/utils.py" "${TASK_DIR}/utils.py"
sed -e "s#^    test: .*#    test: ${SCANQA_JSONL}#" \
    -e 's/^task: scanqa$/task: scanqa_mope_new/' \
    "${GUIDE_LMMS_EVAL}/lmms_eval/tasks/scanqa/scanqa.yaml" \
    > "${TASK_DIR}/scanqa.yaml"
cd "${GUIDE_LMMS_EVAL}"
exec > >(tee "${LOG_FILE}") 2>&1
"${COMMAND[@]}"

mapfile -t RESULT_FILES < <(find "${RUN_OUTPUT_DIR}" -type f -name '*_results.json' -print)
mapfile -t SAMPLE_FILES < <(find "${RUN_OUTPUT_DIR}" -type f -name '*_samples_scanqa_mope_new.jsonl' -print)
[[ "${#RESULT_FILES[@]}" -eq 1 ]] || {
