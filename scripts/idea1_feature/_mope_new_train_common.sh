...
echo "Experiment=${MOPE_NEW_EXPERIMENT} init_checkpoint=${GUIDE_CKPT_PATH} train_llm=${TUNE_LLM} train_projector=True train_mope=False train_other=$([[ "${MOPE_NEW_EXPERIMENT}" == "e04a-new" ]] && echo False || echo True) gate=False CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES} per_device_batch=${PER_DEVICE_TRAIN_BATCH_SIZE} grad_accum=${GRAD_ACCUM} effective_batch=${EFFECTIVE_BATCH} learning_rate=${LEARNING_RATE} lr_scheduler_type=${LR_SCHEDULER_TYPE}"
