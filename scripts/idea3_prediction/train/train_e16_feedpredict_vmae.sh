#!/bin/bash
# =============================================================================
# E-16_vmae (paper-2 Group 2b, D-26/D-27): feed + predict, VideoMAE frozen encoder.
#
# STRICT one-variable clone of E-16 (train_e16_lfp_both.sh): same E-00b warm-start
# checkpoint, same two-stage config, same VGGT geometry, same feed + LFP head,
# same lambda default. The ONLY difference vs E-16 is the frozen encoder:
#   --mope_encoder_type videomae   (plain no-MoE VideoMAE ViT-B/16 drop-in;
#                                    default 'mope' = byte-for-byte E-16)
# and MOPE_CKPT_PATH points at the VideoMAE weights. Both encoders emit
# [B,784,768] time-major latents, so feed / LFP head / bin split all unchanged.
#
# Purpose (Group 2b): does feed+predict ALSO hurt VSI/appearance_order on
# VideoMAE? -> whether the predict harm is a method-level effect (留待 MoPE V2
# 复核) vs an encoder-specific one. Judge with组内 increment (E-16_vmae vs
# E-03a_vmae), NOT vs the MoPE arm (CONCERN-4). GAP-1 (UCF101 ckpt) applies.
#
# Inherited E-16 flags (unchanged):
...
# Override lambda without editing this file: LFP_WEIGHT=0.03 bash train_e16_lfp_both.sh
#
...
# VideoMAE frozen-encoder weights (E-16_vmae). Default = training-cluster mount;
# on THIS box override to /work/hdd/bgkq/lwu9/mope-jepa/pretrained/videomaev2_base.pth
...
if [ "${MODEL_SIZE}" = "4b" ]; then
    ...
    output_dir="${OUTPUT_DIR:-${SPACE_OUTPUT_ROOT}/train/e16_feedpredict_vmae_4b}"
    run_name="space_e16_feedpredict_vmae_4b_lr1e-5"
    # RESUME TRAP GUARD: resume ONLY from THIS experiment's own output_dir.
    RESUME_FROM_CHECKPOINT=${RESUME_FROM_CHECKPOINT:-${SPACE_OUTPUT_ROOT}/train/e16_feedpredict_vmae_4b/checkpoint-4000}
    ...
    output_dir="${OUTPUT_DIR:-${SPACE_OUTPUT_ROOT}/train/e16_feedpredict_vmae_8b}"
    run_name="space_e16_feedpredict_vmae_8b_lr1e-5"
    echo "WARNING: 8B E-16_vmae is experimental — monitor VRAM usage carefully." >&2
else
...
# Launch
# ---------------------------------------------------------------------------
LOG_FILE="${LOG_DIR}/e16_feedpredict_vmae_${MODEL_SIZE}_$(date +%Y%m%d_%H%M%S).log"

echo "=== E-16_vmae Training (MODEL_SIZE=${MODEL_SIZE}) — feed+predict, VideoMAE encoder ==="
