#!/bin/bash
# =============================================================================
# E-15: LFP prediction-only — auxiliary future-latent prediction head, NO feed.
#
...
#   --mope_feed_features False   (do NOT inject MoPE cross-attn residual; the
#                                 crossattn projector is installed only so the
#                                 frozen-MoPE latent cache is populated for the
#                                 LFP target path)
#   --mope_lfp_enable   True     (build + train the 2-layer-MLP LFP head;
...
#   E-15   = predict only   (mope_feed_features=F, mope_lfp_enable=T)  THIS
#   E-16   = feed + predict (mope_feed_features=T, mope_lfp_enable=T)
#
# Usage:
#   MODEL_SIZE=4b bash train_e15_lfp_predonly.sh   # default
#   MODEL_SIZE=8b bash train_e15_lfp_predonly.sh
# =============================================================================
set -e
...
if [ "${MODEL_SIZE}" = "4b" ]; then
    ...
    output_dir="${OUTPUT_DIR:-${SPACE_OUTPUT_ROOT}/train/e15_lfp_predonly_4b}"
    run_name="space_e15_lfp_predonly_4b_lr1e-5"
    RESUME_FROM_CHECKPOINT=${RESUME_FROM_CHECKPOINT:-${SPACE_OUTPUT_ROOT}/train/e15_lfp_predonly_4b/checkpoint-4000}
    ...
    output_dir="${OUTPUT_DIR:-${SPACE_OUTPUT_ROOT}/train/e15_lfp_predonly_8b}"
    run_name="space_e15_lfp_predonly_8b_lr1e-5"
    echo "WARNING: 8B E-15 is experimental — monitor VRAM usage carefully." >&2
else
...
# Launch
# ---------------------------------------------------------------------------
LOG_FILE="${LOG_DIR}/e15_lfp_predonly_${MODEL_SIZE}_$(date +%Y%m%d_%H%M%S).log"

echo "=== E-15 Training (MODEL_SIZE=${MODEL_SIZE}) — LFP prediction-only ==="
