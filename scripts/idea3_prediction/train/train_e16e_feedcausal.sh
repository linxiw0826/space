#!/bin/bash
# =============================================================================
# E-16e: token-shift LFP (feed + predict) + causal-over-bins mask on the FEED
# cross-attn.
#
...
# E-16e flag --mope_feed_causal_mask (default False in argument.py). When the
# flag is False the feed cross-attn is bidirectional and the loss is
# byte-for-byte identical to E-16; this script sets it True.
#
# Diagnosis (paper2_design.md / decisions.md): E-16's feed cross-attn is
# BIDIRECTIONAL — every image token (query) attends to ALL MoPE K/V tokens
# across all 4 time-bins, so future-bin motion leaks backward into earlier
# frames and blurs temporal order (V-CORE / Video-CCAM finding). E-16e adds a
# causal mask over the temporal (bin) axis: an image token of frame f (mapped to
# bin b_f via the faithful _group_llm_frames_to_bins mapping) attends ONLY to
# MoPE K/V of bins <= b_f; future-bin K/V is masked with -inf. This is
# Video-CCAM's get_ccam causal mask adapted to our layout (queries = per-frame
# image tokens, K/V groups = MoPE time-bins). Single variable vs E-16.
#
# The ONLY differences vs train_e16_lfp_both.sh:
#   --mope_feed_causal_mask True              (the single new variable)
#   EXP_NAME / output_dir / run_name -> e16e_feedcausal_4b
# LFP hyperparameters use the E-16 values (feed=T, lfp=T, lambda=0.05 default).
# Before a full run, read the rank0 '[LFP DIAG]' print (first 3 steps) and tune
# --mope_lfp_weight by the ratio it reports (target ratio ~0.05-0.2).
# Override lambda without editing this file: LFP_WEIGHT=0.03 bash train_e16e_feedcausal.sh
#
# Paper-2 Stage-1 score-first roadmap (paper2_design.md §4.1):
#   E-16   = feed + predict                        (the regressed baseline)
#   E-16e  = feed + predict + feed-causal mask     (THIS)
#
# Usage:
#   MODEL_SIZE=4b bash train_e16e_feedcausal.sh   # default
#   MODEL_SIZE=8b bash train_e16e_feedcausal.sh
# =============================================================================
set -e
...
if [ "${MODEL_SIZE}" = "4b" ]; then
    ...
    output_dir="${OUTPUT_DIR:-${SPACE_OUTPUT_ROOT}/train/e16e_feedcausal_4b}"
    run_name="space_e16e_feedcausal_4b_lr1e-5"
    ...
    RESUME_FROM_CHECKPOINT=${RESUME_FROM_CHECKPOINT:-${SPACE_OUTPUT_ROOT}/train/e16e_feedcausal_4b/checkpoint-4000}
    ...
    output_dir="${OUTPUT_DIR:-${SPACE_OUTPUT_ROOT}/train/e16e_feedcausal_8b}"
    run_name="space_e16e_feedcausal_8b_lr1e-5"
    echo "WARNING: 8B E-16e is experimental — monitor VRAM usage carefully." >&2
else
...
# ---------------------------------------------------------------------------
# Training arguments
# (identical to E-16 except: output_dir + --mope_feed_causal_mask)
# ---------------------------------------------------------------------------
args="
...
# Launch
# ---------------------------------------------------------------------------
LOG_FILE="${LOG_DIR}/e16e_feedcausal_${MODEL_SIZE}_$(date +%Y%m%d_%H%M%S).log"

echo "=== E-16e Training (MODEL_SIZE=${MODEL_SIZE}) — LFP feed+predict + feed-causal mask ==="
echo "Output : ${output_dir}"
echo "Log    : ${LOG_FILE}"
echo "Fusion : crossattn, batch=${batch_size}, accum=${grad_accum_steps}"
echo "Feed   : ON  (mope_feed_features=True)"
echo "Causal : ON  (mope_feed_causal_mask=True, frame f attends MoPE bins <= b_f)"
