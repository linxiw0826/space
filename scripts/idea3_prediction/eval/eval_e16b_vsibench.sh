#!/bin/bash
# =============================================================================
# Evaluate E-16b (LFP feed + predict + VICReg anti-collapse) on VSIBench.
# Model type: qwen3_vl_mope_crossattn (MoPE-ON inference).
# (forked from eval_e16_vsibench.sh — only EXP_NAME / ckpt / output differ.)
#
# ATTRIBUTION RATIONALE (paper-2 Stage-1 eval design):
# E-16b is trained with --mope_feed_features=True (cross-attn residual injection
# ON), exactly like E-16; the VICReg anti-collapse regularizer is TRAINING-ONLY
# (no head/loss at inference). So the MoPE-ON inference path
...
# injected exactly as in E-03a/E-16. The only difference from E-16 is that the
# LLM representations were trained with the added VICReg decoupling objective.
#
# Paper-2 Stage-1 attribution table:
# | experiment | train feed | train LFP | train reg | eval model               | eval MoPE |
# |------------|:---------:|:---------:|:---------:|--------------------------|:---------:|
# | E-16       | ON        | ON        | OFF       | qwen3_vl_mope_crossattn  | ON        |
# | E-16b      | ON        | ON        | ON(VICReg)| qwen3_vl_mope_crossattn  | ON        |  <-- THIS
#
# Usage:
#   bash eval_e16b_vsibench.sh [CKPT_PATH] [EXP_NAME]
#
...
case "${MODEL_SIZE}" in
    4b)
        DEFAULT_CKPT_PATH="${SPACE_OUTPUT_ROOT}/train/e16b_anticollapse_4b"
        DEFAULT_EXP_NAME="e16b_anticollapse_4b"
        ;;
    8b)
        DEFAULT_CKPT_PATH="${SPACE_OUTPUT_ROOT}/train/e16b_anticollapse_8b"
        DEFAULT_EXP_NAME="e16b_anticollapse_8b"
        ;;
    *)
...
mkdir -p "${LOG_DIR}"
LOG_FILE="${LOG_DIR}/e16b_vsibench_${MODEL_SIZE}_$(date +%Y%m%d_%H%M%S).log"
...
# E-16b's trained cross-attn projector loads (strict=False keeps matched keys);
...
# Status output
# ---------------------------------------------------------------------------
echo "=== VSIBench Evaluation (E-16b LFP feed+predict+VICReg, MoPE-ON inference) ==="
