#!/bin/bash
# =============================================================================
# Evaluate E-16t (LFP feed + predict + Cosmos feed temporal PE) on VSIBench.
# Model type: qwen3_vl_mope_crossattn (MoPE-ON inference).
#
# E-16t train/test consistency (★ MUST): the checkpoint was TRAINED with the
# feed temporal PE (--mope_feed_temporal_pe True), and the feed cross-attn runs
# at inference, so eval MUST also build the temporal-PE projector. This is gated
# by MOPE_FEED_TEMPORAL_PE=1 (exported below, default ON). Leaving it OFF would
# drop the temporal code at test time -> train/test mismatch -> invalid scores.
# Set MOPE_FEED_TEMPORAL_PE=1 ONLY for E-16t checkpoints.
#
# ATTRIBUTION RATIONALE (paper-2 Stage-1 2x2 eval design):
# E-16 is trained with --mope_feed_features=True (cross-attn residual
# injection ON). The cross-attn projector's k/v/out_proj ARE trained to
# inject useful information into image_embeds, so the MoPE-ON inference path
# (qwen3_vl_mope_crossattn) is the correct evaluation mode — the projector
# weights load (strict=False keeps the matched keys) and the residual is
# injected exactly as it was during E-03a. The only difference from E-03a is
# that the LLM representations were ADDITIONALLY reshaped by the LFP
# auxiliary objective during E-16 training.
#
...
#
# Usage:
#   bash eval_e16_vsibench.sh [CKPT_PATH] [EXP_NAME]
#
...
# E-16t: build the temporal-PE projector at eval (MUST match the checkpoint).
# Default 1 = ON; keep ON for every E-16t checkpoint.
export MOPE_FEED_TEMPORAL_PE=${MOPE_FEED_TEMPORAL_PE:-1}
...
case "${MODEL_SIZE}" in
    4b)
        DEFAULT_CKPT_PATH="${SPACE_OUTPUT_ROOT}/train/e16t_temporal_4b"
        DEFAULT_EXP_NAME="e16t_temporal_4b"
        ;;
    8b)
        DEFAULT_CKPT_PATH="${SPACE_OUTPUT_ROOT}/train/e16t_temporal_8b"
        DEFAULT_EXP_NAME="e16t_temporal_8b"
        ;;
    *)
...
mkdir -p "${LOG_DIR}"
LOG_FILE="${LOG_DIR}/e16t_vsibench_${MODEL_SIZE}_$(date +%Y%m%d_%H%M%S).log"
...
# Status output
# ---------------------------------------------------------------------------
echo "=== VSIBench Evaluation (E-16t LFP feed+predict + temporal PE, MoPE-ON) ==="
echo "TemporalPE : MOPE_FEED_TEMPORAL_PE=${MOPE_FEED_TEMPORAL_PE}  (MUST be 1 for E-16t train/test consistency)"
