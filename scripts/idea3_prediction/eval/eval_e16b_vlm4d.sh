#!/bin/bash
# =============================================================================
# Evaluate E-16b (LFP feed + predict + VICReg anti-collapse) on VLM4D.
# Model type: qwen3_vl_mope_crossattn (MoPE-ON inference).
# (forked from eval_e16_vlm4d.sh — only EXP_NAME / ckpt / output differ.)
#
...
# See eval_e16b_vsibench.sh header — E-16b is trained with feed=True (same as
# E-16) and the VICReg anti-collapse regularizer is TRAINING-ONLY, so the
...
#
# Usage:
#   bash eval_e16b_vlm4d.sh [CKPT_PATH] [EXP_NAME]
#
...
case "${MODEL_SIZE}" in
    4b)
        ...
        DEFAULT_EXP_NAME="e16b_anticollapse_4b_vlm4d"
        ;;
    8b)
        DEFAULT_CKPT_PATH="${SPACE_OUTPUT_ROOT}/train/e16b_anticollapse_8b"
        DEFAULT_EXP_NAME="e16b_anticollapse_8b_vlm4d"
        ;;
    *)
...
mkdir -p "${LOG_DIR}"
LOG_FILE="${LOG_DIR}/e16b_vlm4d_${MODEL_SIZE}_$(date +%Y%m%d_%H%M%S).log"
...
# Status output
# ---------------------------------------------------------------------------
echo "=== VLM4D Evaluation (E-16b LFP feed+predict+VICReg, MoPE-ON inference) ==="
