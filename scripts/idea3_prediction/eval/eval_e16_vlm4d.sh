#!/bin/bash
# =============================================================================
# Evaluate E-16 (LFP feed + predict) on VLM4D.
...
case "${MODEL_SIZE}" in
    4b)
        ...
        DEFAULT_EXP_NAME="e16_lfp_both_4b_vlm4d"
        ;;
    8b)
        DEFAULT_CKPT_PATH="${SPACE_OUTPUT_ROOT}/train/e16_lfp_both_8b"
        DEFAULT_EXP_NAME="e16_lfp_both_8b_vlm4d"
        ;;
    *)
...
mkdir -p "${LOG_DIR}"
LOG_FILE="${LOG_DIR}/e16_vlm4d_${MODEL_SIZE}_$(date +%Y%m%d_%H%M%S).log"
...
# Status output
# ---------------------------------------------------------------------------
echo "=== VLM4D Evaluation (E-16 LFP feed+predict, MoPE-ON inference) ==="
