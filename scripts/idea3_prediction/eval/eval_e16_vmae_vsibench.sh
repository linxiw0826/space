#!/bin/bash
# =============================================================================
# Evaluate E-16_vmae (feed + predict, VideoMAE frozen encoder) on VSIBench.
...
case "${MODEL_SIZE}" in
    4b)
        ...
        DEFAULT_EXP_NAME="e16_feedpredict_vmae_4b"
        ;;
    8b)
        DEFAULT_CKPT_PATH="${SPACE_OUTPUT_ROOT}/train/e16_feedpredict_vmae_8b"
        DEFAULT_EXP_NAME="e16_feedpredict_vmae_8b"
        ;;
    *)
...
mkdir -p "${LOG_DIR}"
LOG_FILE="${LOG_DIR}/e16_vmae_vsibench_${MODEL_SIZE}_$(date +%Y%m%d_%H%M%S).log"
...
# Status output
# ---------------------------------------------------------------------------
echo "=== VSIBench Evaluation (E-16_vmae feed+predict, VideoMAE encoder, MoPE-ON) ==="
