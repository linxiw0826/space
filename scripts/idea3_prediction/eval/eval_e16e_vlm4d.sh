#!/bin/bash
# =============================================================================
# Evaluate E-16e (LFP feed + predict + feed-causal mask) on VLM4D.
# Model type: qwen3_vl_mope_crossattn (MoPE-ON inference).
# (forked from eval_e16b_vlm4d.sh — EXP_NAME / ckpt / output differ, PLUS
...
# See eval_e16e_vsibench.sh header — E-16e is trained with feed=True (same as
# E-16) PLUS the feed-causal mask (a training-only argument that shapes the
# trained cross-attn); the LFP predict head is TRAINING-ONLY, so the cross-attn
# projector is trained and the MoPE-ON inference path (qwen3_vl_mope_crossattn)
# with the feed-causal mask ON is the correct evaluation mode.
#
...
#
# Usage:
#   bash eval_e16e_vlm4d.sh [CKPT_PATH] [EXP_NAME]
#
...
case "${MODEL_SIZE}" in
    4b)
        ...
        DEFAULT_EXP_NAME="e16e_feedcausal_4b_vlm4d"
        ;;
    8b)
        DEFAULT_CKPT_PATH="${SPACE_OUTPUT_ROOT}/train/e16e_feedcausal_8b"
        DEFAULT_EXP_NAME="e16e_feedcausal_8b_vlm4d"
        ;;
    *)
...
mkdir -p "${LOG_DIR}"
LOG_FILE="${LOG_DIR}/e16e_vlm4d_${MODEL_SIZE}_$(date +%Y%m%d_%H%M%S).log"
...
# Status output
# ---------------------------------------------------------------------------
echo "=== VLM4D Evaluation (E-16e LFP feed+predict+causal-mask, MoPE-ON inference) ==="
