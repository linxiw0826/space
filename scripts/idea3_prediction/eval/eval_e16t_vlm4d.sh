#!/bin/bash
# =============================================================================
# Evaluate E-16t (LFP feed + predict + Cosmos feed temporal PE) on VLM4D.
# Model type: qwen3_vl_mope_crossattn (MoPE-ON inference).
# (forked from eval_e16t_vsibench.sh — only the task + data paths differ).
#
# E-16t train/test consistency: MOPE_FEED_TEMPORAL_PE=1 (default ON) builds the
# temporal-PE projector to match the checkpoint. See eval_e16t_vsibench.sh.
#
...
case "${MODEL_SIZE}" in
    4b)
        ...
        DEFAULT_EXP_NAME="e16t_temporal_4b_vlm4d"
        ;;
    8b)
        DEFAULT_CKPT_PATH="${SPACE_OUTPUT_ROOT}/train/e16t_temporal_8b"
        DEFAULT_EXP_NAME="e16t_temporal_8b_vlm4d"
        ;;
    *)
...
mkdir -p "${LOG_DIR}"
LOG_FILE="${LOG_DIR}/e16t_vlm4d_${MODEL_SIZE}_$(date +%Y%m%d_%H%M%S).log"
...
# Status output
# ---------------------------------------------------------------------------
echo "=== VLM4D Evaluation (E-16t LFP feed+predict + temporal PE, MoPE-ON) ==="
