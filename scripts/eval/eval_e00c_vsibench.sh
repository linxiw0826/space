#!/bin/bash
# =============================================================================
# Evaluate E-00c (Q-Former projector only) on VSIBench
# Model type: qwen3_vl_mope_qformer
#
# Usage:
#   bash eval_e00c_vsibench.sh [CKPT_PATH] [EXP_NAME]
#
# Arguments (optional — derived from MODEL_SIZE when omitted):
#   CKPT_PATH  — path to the E-00c checkpoint directory to evaluate
...
case "${MODEL_SIZE}" in
    4b)
        DEFAULT_CKPT_PATH="/home/nvme03/wlx/Space_sensing/output/train/e00c_mope_qformer_projector_only_4b"
        DEFAULT_EXP_NAME="e00c_mope_qformer_projector_only_4b"
        ;;
    8b)
        DEFAULT_CKPT_PATH="/home/nvme03/wlx/Space_sensing/output/train/e00c_mope_qformer_projector_only_8b"
        DEFAULT_EXP_NAME="e00c_mope_qformer_projector_only_8b"
        ;;
    *)
...
mkdir -p "${LOG_DIR}"
LOG_FILE="${LOG_DIR}/e00c_vsibench_${MODEL_SIZE}_$(date +%Y%m%d_%H%M%S).log"
...
# Status output
# ---------------------------------------------------------------------------
echo "=== VSIBench Evaluation (E-00c MoPE QFormer projector only) ==="
