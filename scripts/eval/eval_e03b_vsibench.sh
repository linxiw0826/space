#!/bin/bash
# =============================================================================
# Evaluate E-03b (Q-Former two-stage) on VSIBench
# Model type: qwen3_vl_mope_qformer
#
# Usage:
#   bash eval_e03b_vsibench.sh [CKPT_PATH] [EXP_NAME]
#
# Arguments (optional — derived from MODEL_SIZE when omitted):
#   CKPT_PATH  — path to the E-03b checkpoint directory to evaluate
...
case "${MODEL_SIZE}" in
    4b)
        DEFAULT_CKPT_PATH="/home/nvme03/wlx/Space_sensing/output/train/e03b_mope_qformer_two_stage_4b"
        DEFAULT_EXP_NAME="e03b_mope_qformer_two_stage_4b"
        ;;
    8b)
        DEFAULT_CKPT_PATH="/home/nvme03/wlx/Space_sensing/output/train/e03b_mope_qformer_two_stage_8b"
        DEFAULT_EXP_NAME="e03b_mope_qformer_two_stage_8b"
        ;;
    *)
...
mkdir -p "${LOG_DIR}"
LOG_FILE="${LOG_DIR}/e03b_vsibench_${MODEL_SIZE}_$(date +%Y%m%d_%H%M%S).log"
...
# Status output
# ---------------------------------------------------------------------------
echo "=== VSIBench Evaluation (E-03b MoPE QFormer two-stage) ==="
