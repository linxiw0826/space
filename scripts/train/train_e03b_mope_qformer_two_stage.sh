...
#   MODEL_SIZE=8b bash train_e03b_mope_qformer_two_stage.sh
#
...
if [ "${MODEL_SIZE}" = "4b" ]; then
    ...
    GUIDE_CKPT_PATH=${GUIDE_CKPT_PATH:-/home/nvme03/wlx/Space_sensing/output/train/e00c_mope_qformer_projector_only_4b}
    output_dir="${OUTPUT_DIR:-/home/nvme03/wlx/Space_sensing/output/train/e03b_mope_qformer_two_stage_4b}"
    run_name="space_e03b_mope_qformer_two_stage_4b_lr1e-5"
    RESUME_FROM_CHECKPOINT=${RESUME_FROM_CHECKPOINT:-/home/nvme03/wlx/Space_sensing/output/train/e03b_mope_qformer_two_stage_4b/checkpoint-4000}
elif [ "${MODEL_SIZE}" = "8b" ]; then
    batch_size=1
    grad_accum_steps=16
    # 8B requires ZeRO-3 to fit on 8x H800 GPUs.
    DEEPSPEED_CONFIG=${DEEPSPEED_CONFIG:-${SPACE_ROOT}/configs/zero3.json}
    GUIDE_CKPT_PATH=${GUIDE_CKPT_PATH:-/home/nvme03/wlx/Space_sensing/output/train/e00c_mope_qformer_projector_only_8b}
    output_dir="${OUTPUT_DIR:-/home/nvme03/wlx/Space_sensing/output/train/e03b_mope_qformer_two_stage_8b}"
    run_name="space_e03b_mope_qformer_two_stage_8b_lr1e-5"
    echo "WARNING: 8B E-03b is experimental — monitor VRAM usage carefully." >&2
else
...
# down to a fixed 32-token summary prepended to the LLM sequence.
...
# Launch
# ---------------------------------------------------------------------------
LOG_FILE="${LOG_DIR}/e03b_mope_qformer_two_stage_${MODEL_SIZE}_$(date +%Y%m%d_%H%M%S).log"

echo "=== E-03b Training (MODEL_SIZE=${MODEL_SIZE}) ==="
