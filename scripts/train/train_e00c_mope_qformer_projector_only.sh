#!/bin/bash
# =============================================================================
# E-00c: GUIDE + MoPE Q-Former fusion, projector only (unified 4B / 8B)
#
# LLM-frozen control experiment for E-02d (Q-Former fusion).
# Only MoPEProjectorQFormer is trained on VSI-590K.
# Trainable: MoPEProjectorQFormer only, LLM frozen.
#
# Usage:
#   MODEL_SIZE=4b bash train_e00c_mope_qformer_projector_only.sh   # default
#   MODEL_SIZE=8b bash train_e00c_mope_qformer_projector_only.sh
#
# Supported MODEL_SIZE values: 4b  8b
#
# Key differences from E-02d:
#   - --tune_mm_llm False             (LLM frozen; only MoPEProjectorQFormer trains)
...
# Launch
# ---------------------------------------------------------------------------
LOG_FILE="${LOG_DIR}/e00c_mope_qformer_projector_only_${MODEL_SIZE}_$(date +%Y%m%d_%H%M%S).log"

echo "=== E-00c Training (MODEL_SIZE=${MODEL_SIZE}) ==="
echo "Output : ${output_dir}"
echo "Log    : ${LOG_FILE}"
echo "Fusion : qformer, N_queries=32, batch=${batch_size}, accum=${grad_accum_steps}"
echo "Trainable: MoPEProjectorQFormer only, LLM frozen"
