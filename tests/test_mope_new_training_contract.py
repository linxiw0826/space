...
def test_e04a_lr1e3_bs2_launcher_preserves_effective_batch_and_isolated_output(tmp_path):
    root = Path(__file__).resolve().parents[1]
    output_root = tmp_path / "output"
    env = {
        **os.environ,
    ...
    result = subprocess.run(
        [
            ...
            str(
                ...
                "train_e04a_mope_new_e01_projector_only_lr1e3_bs2.sh"
            ),
        ],
        cwd=root,
        env=env,
    ...
    assert "--learning_rate 1e-3" in result.stdout
    assert "effective_batch=48" in result.stdout
    assert "learning_rate=1e-3" in result.stdout
    assert str(
        output_root / "train/e04a_mope_new_e01_projector_only_lr1e3_bs2_4b"
    ) in result.stdout
...
def test_e04a_dry_run_is_e01_initialized_projector_only(tmp_path):
    ...
    assert "per_device_batch=1" in result.stdout
    assert "grad_accum=12" in result.stdout
    assert "effective_batch=48" in result.stdout
