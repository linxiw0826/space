...
def _read_video_paths(source):
    ...
    if source.is_file() and source.suffix.lower() == ".txt":
        with source.open("r", encoding="utf-8") as handle:
            return [("video", str(Path(line.strip())), None) for line in handle if line.strip()]
        ...
        with source.open("r", encoding="utf-8") as handle:
            for line in handle:
                if not line.strip():
                    continue
                row = json.loads(line)
                frame_dir = row.get("frame_dir") or row.get("sample_dir")
                rel_dir = row.get("frame_rel_dir") or row.get("frames_rel_dir") or row.get("sample_rel_dir")
                if not frame_dir and rel_dir:
                    frame_dir = str(source.parent / rel_dir)
                if frame_dir:
                    rows.append(("frames", frame_dir, row))
                elif row.get("source_video"):
                    rows.append(("video", row["source_video"], row))
        return rows
    if source.is_dir():
        return [("video", str(path), None) for path in sorted(
...
class VideoPretrainDataset(torch.utils.data.Dataset):
    def __init__(self, datasets_root, transform, num_frames=16,
        ...
        primary = [item for item in _read_video_paths(datasets_root) if Path(item[1]).exists()]
        self.clips.extend((item, 0) for item in primary)

        general = []
        if general_data:
            general = [item for item in _read_video_paths(general_data) if Path(item[1]).exists()]
            if general_max > 0:
                general = general[:general_max]
            self.clips.extend((item, 1) for item in general)
    ...
    def _load_preextracted_frames(self, frame_dir):
        frame_dir = Path(frame_dir)
        paths = [frame_dir / f"{i:06d}.jpg" for i in range(self.num_frames)]
        if not all(path.is_file() for path in paths):
            paths = sorted(frame_dir.glob("*.jpg"))[:self.num_frames]
        if len(paths) < self.num_frames:
            raise RuntimeError(f"Expected {self.num_frames} frames in {frame_dir}, found {len(paths)}")
        return [Image.open(path).convert("RGB") for path in paths]

    def _load_frames(self, clip):
        kind, path, _meta = clip
        if kind == "frames":
            return self._load_preextracted_frames(path)
    ...
    def __getitem__(self, index):
        clip, binary_label = self.clips[index]
        try:
            images = self._load_frames(clip)
        except Exception as error:
            print(f"[Dataset] Load failed ({clip[1]}): {error}; retrying")
