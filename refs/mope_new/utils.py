...
class SmoothedValue(object):
    ...
    @property
    def min(self):
        return min(self.deque) if len(self.deque) > 0 else 0.0
...
class NativeScalerWithGradNormCount:
    ...
    def __init__(self):
        # PyTorch 2.x 推荐 torch.amp.GradScaler('cuda')
...
def multiple_pretrain_samples_collate(batch, fold=False):
    ...
    if n_fields == 3:
        ...
        (process_data, encoder_mask, decoder_mask,
         physics_label, physics_label_soft) = zip(*batch)
        lbl_flat = [x for sub in physics_label for x in sub]
        soft_flat = [x for sub in physics_label_soft for x in sub]
        tail = (default_collate(lbl_flat), default_collate(soft_flat))
    else:
        raise ValueError(
            f"multiple_pretrain_samples_collate: expected 3 or 5 fields per "
            f"sample, got {n_fields}")
    ...
    if fold:
        base = ([process_data], encoder_mask, decoder_mask)
    else:
        base = (process_data, encoder_mask, decoder_mask)
    if tail is not None:
        return base + tail
