...
class RandomTemporalMaskingGenerator:
    """Randomly mask whole temporal steps.

    For VideoMAE/MoPE token grid [T, H, W], this keeps complete spatial frames
    at a few temporal tokens and masks the remaining temporal tokens.
    Example: T=8, mask_ratio=0.625 -> mask 5 T steps, keep 3 T steps.
    """
    def __init__(self, input_size, mask_ratio):
        if not isinstance(input_size, tuple):
            raise ValueError("input_size should be a tuple: (T, H, W)")
        self.frames, self.height, self.width = input_size
        self.num_patches = self.frames * self.height * self.width

        self.num_mask_t = int(round(self.frames * mask_ratio))
        self.num_mask_t = max(1, min(self.frames - 1, self.num_mask_t))
        self.num_masks = self.num_mask_t * self.height * self.width

    def __repr__(self):
        return "Random Temporal Masking: total patches {}, mask patches {}, mask_t {}/{}".format(
            self.num_patches, self.num_masks, self.num_mask_t, self.frames
        )

    def __call__(self):
        import numpy as np
        mask_t = np.hstack([
            np.zeros(self.frames - self.num_mask_t, dtype=np.int64),
            np.ones(self.num_mask_t, dtype=np.int64),
        ])
        np.random.shuffle(mask_t)
        mask = np.repeat(mask_t[:, None, None], self.height, axis=1)
        mask = np.repeat(mask, self.width, axis=2)
        return mask.flatten()
